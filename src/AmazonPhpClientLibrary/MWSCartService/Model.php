...
/**
 * MWSCartService_Model - base class for all model classes
 */
abstract class MWSCartService_Model
{
    ...
    */
    ...
    protected function _toXMLFragment() 
    {
        ...
        foreach ($this->_fields as $fieldName => $field) {
            ...
            if (!is_null($fieldValue) && $field['FieldType'] != "MWSCartService_Model_ResponseHeaderMetadata") {
                $fieldType = $field['FieldType'];
                if (is_array($fieldType)) {
                    if ($fieldType[0] == "object") {
    ...
    {
        return preg_match("/^MWSCartService_/", $fieldType);
    }
